...
class SassExtentionsCompassFunctionsInlineData {
	public function inline_image($path, $mime_type = null) {
		$path = $path->value;
		$real_path = Phamlp_Sass_Extension_compass_Config::config('images_path').DIRECTORY_SEPARATOR.$path;
	...
	public function inline_font_files() {
		if (func_num_args() % 2)
			throw new Phamlp_Sass_Script_FunctionException('An even number of arguments must be passed to inline_font_files()', array(), Phamlp_Sass_Script_Parser::$context->node);

		$args = func_get_args();
		$files = array();
		while ($args) {
			$path = array_shift($args);
			$real_path = Phamlp_Sass_Extension_compass_Config::config('fonts_path').DIRECTORY_SEPARATOR.$path->value;
	...
	private function compute_mime_type($path, $mime_type = null) {
		...
		switch (true) {
			...
			case preg_match('/\.([a-zA-Z]+)$/i', $path, $matches):
				...
				break;
			default:
				throw new Phamlp_Sass_Script_FunctionException('Unable to determine mime type for {what}, please specify one explicitly', array('{what}'=>$path), Phamlp_Sass_Script_Parser::$context->node);
				break;
		}
	}
	...
	private function data($real_path) {
		if (file_exists($real_path)) {
			...
			throw new Phamlp_Sass_Script_FunctionException('Unable to find {what}: {filename}', array('{what}'=>'file', '{filename}'=>$real_path), Phamlp_Sass_Script_Parser::$context->node);
	}
}
