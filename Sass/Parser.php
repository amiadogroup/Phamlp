...
class Phamlp_Sass_Parser {
	...
	public function __get($name) {
		...
		throw new Phamlp_Sass_Exception('No getter function for {what}', array('{what}'=>$name));
	}
	 ...
	 * @param string name of source file or Sass source
	 * @return Phamlp_Sass_Tree_Node_Root Root node of document tree
	 */
	public function parse($source, $isFile = true) {
		if ($isFile) {
			...
			elseif ($this->syntax !== Phamlp_Sass_File::SASS && $this->syntax !== Phamlp_Sass_File::SCSS) {
				throw new Phamlp_Sass_Exception('Invalid {what}', array('{what}'=>'syntax option'));
			}
	 ...
	 * @return Phamlp_Sass_Tree_Node_Root the root of this document tree
	 */
	private function toTree($source) {
		...
		unset($source);
		$root = new Phamlp_Sass_Tree_Node_Root($this);
		$this->buildTree($root);
		return $root;
	}
	...
	private function getNode($node) {
		...
		switch (true) {
			...
			case Phamlp_Sass_Tree_Node_Mixin_Definition::isa($token):
				...
				break;
			case Phamlp_Sass_Tree_Node_Mixin::isa($token):
				if ($this->syntax === Phamlp_Sass_File::SCSS) {
					throw new Phamlp_Sass_Exception('Mixin {which} shortcut not allowed in SCSS', array('{which}'=>'include'), $this);
				}
		...
		while (is_null($token) && !empty($this->source)) {
			...
			if ($statement[0] === self::BEGIN_COMMENT) {
				...
				elseif (substr($statement, 0, strlen(self::BEGIN_CSS_COMMENT))
					...
					throw new Phamlp_Sass_Exception('Illegal comment type', array(), $this);
				}
			}
	...
	private function getLevel($source) {
		...
		if (!is_int($level) ||
			...
			$this->source = $source;
			throw new Phamlp_Sass_Exception('Invalid indentation', array(), $this);
		}
		return $level;
	}
			...
			switch ($c) {
				case self::BEGIN_COMMENT:	
					...
					elseif (substr($this->source, $srcpos-1, strlen(self::BEGIN_CSS_COMMENT))
							=== self::BEGIN_CSS_COMMENT) {
						if (ltrim($statement)) {
							throw new Phamlp_Sass_Exception('Invalid {what}', array('{what}'=>'comment'), (object) array(
								'source' => $statement,
								'filename' => $this->filename,
								'line' => $this->line,
							));
	 ...
	 * @throws Phamlp_Sass_Exception if the indent is mixed or
		...
		foreach ($this->source as $l=>$source) {
			if (!empty($source) && in_array($source[0], $this->indentChars)) {
				...
				if ($i < $len && in_array($source[$i], $this->indentChars)) {
					$this->line = ++$l;
					$this->source = $source;
					throw new Phamlp_Sass_Exception('Mixed indentation not allowed', array(), $this);
				}
