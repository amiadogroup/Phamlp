...
/* SVN FILE: $Id$ */
/**
 * Phamlp_Sass_Tree_Node_Rule class file.
...
/**
 * Phamlp_Sass_Tree_Node_Rule class.
 * Represents a CSS rule.
 * @package			PHamlP
 * @subpackage	Sass.tree
 */
class Phamlp_Sass_Tree_Node_Rule extends Phamlp_Sass_Tree_Node {
	const MATCH = '/^(.+?)(?:\s*\{)?$/';
	const SELECTOR = 1;
	const CONTINUED = ',';
	...
	/**
	 * Phamlp_Sass_Tree_Node_Rule constructor.
	 * @param object source token
	 * @param string rule selector
	 * @return Phamlp_Sass_Tree_Node_Rule
	 */
	public function __construct($token) {
		parent::__construct($token);
		preg_match(self::MATCH, $token->source, $matches);
		$this->addSelectors($matches[Phamlp_Sass_Tree_Node_Rule::SELECTOR]);
	}
	...
	public function render() {
		...
		foreach ($this->children as $child) {
			$child->parent = $this;
			if ($child instanceof Phamlp_Sass_Tree_Node_Rule) {
				$rules .= $child->render();
			}
			else {
	...
	protected function getParentSelectors($context) {
		...
		while (!$ancestor instanceof Phamlp_Sass_Tree_Node_Rule && $ancestor->hasParent()) {
			$ancestor = $ancestor->parent;
		}

		if ($ancestor instanceof Phamlp_Sass_Tree_Node_Rule) {
